...
public class Rook extends Piece {
    ...
    public Rook(PositionDto position, Color color, PieceType type, boolean hasMoved) {
        super(position, color, type, hasMoved);
    }

    @Override
    public Collection<MoveDto> getMoves(Chessboard board, boolean validateForCheck) {
        return checkMoves(this.rekursiveStep(this.position, board, List.of(1, -1, 0, 0), List.of(0, 0, -1, 1)), board, validateForCheck);
    }

    @Override
    public Piece clone() {
        return new Rook(position, color, type, hasMoved);
    }
}
