...
public class Bishop extends Piece {
    ...
    public Bishop(PositionDto position, Color color, PieceType type, boolean hasMoved) {
        super(position, color, type, hasMoved);
    }

    @Override
    public Collection<MoveDto> getMoves(Chessboard board, boolean validateForCheck) {
        return checkMoves(this.rekursiveStep(this.position, board, List.of(1, 1, -1, -1), List.of(1, -1, -1, 1)), board, validateForCheck);
    }

    @Override
    public Piece clone() {
        return new Bishop(position, color, type, hasMoved);
    }
}
